...
`define`::
Starts a `define` clause of a xref:3.x@typeql::queries/define.adoc[].

`undefine`::
Starts an `undefine` clause of a xref:3.x@typeql::queries/undefine.adoc[]

`match`::
Starts a `match` clause that xref:3.x@typeql::patterns/matching.adoc[matches existing data] by a pattern in an
...
xref:3.x@typeql::queries/fetch.adoc[Fetch], or
xref:3.x@typeql::queries/get.adoc[Get] query.

`insert`::
Starts an `insert` clause of an xref:3.x@typeql::queries/insert.adoc[].

`delete`::
Starts a `delete` clause of a xref:3.x@typeql::queries/delete.adoc[].

`fetch`::
Starts a `fetch` clause of a xref:3.x@typeql::queries/fetch.adoc[].

`get`::
Starts a `get` clause of a xref:3.x@typeql::queries/get.adoc[].
...
For more information, see the xref:3.x@typeql::statements/sub.adoc[] page.
...
For more information, see the xref:3.x@typeql::statements/type.adoc[] page.

`abstract`::
Specify that a given type is an abstract type. +
For more information, see the xref:3.x@typeql::statements/abstract.adoc[] page.

`owns`::
Specify that a type can own attributes of a given attribute type. +
For more information, see the xref:3.x@typeql::statements/owns.adoc[] page.

`value`::
Specify a value type of an attribute type. +
For more information, see the xref:3.x@typeql::statements/value.adoc[] page.

`relates`::
Add a new role for a relation. +
For more information, see the xref:3.x@typeql::statements/relates.adoc[] page.

`plays`::
Specify that a given type can play a given role (in a given relation). +
For more information, see the xref:3.x@typeql::statements/plays.adoc[] page.

`@key`::
Add a key constraint to ownership of a given attribute type by a type. +
For more information, see the xref:3.x@typeql::statements/key.adoc[] page.

`@unique`::
Add a uniqueness constraint to ownership of a given attribute type by a type. +
For more information, see the xref:3.x@typeql::statements/unique.adoc[] page.

`regex`::
Specify a regular expression that limits values of a given attribute type. +
For more information, see the xref:3.x@typeql::statements/regex.adoc[] page.

`as`::
Override an inherited attribute type ownership or role playing ability. +
For more information, see the xref:3.x@typeql::statements/owns.adoc#_usage_with_override[owns] and
xref:3.x@typeql::statements/plays.adoc#_usage_with_override[plays] pages respectively.

`when`::
Specify a condition of a xref:3.x@typeql::statements/rule.adoc[rule].

`then`::
Specify a conclusion of a xref:3.x@typeql::statements/rule.adoc[rule].
...
For more information, see the xref:3.x@typeql::statements/isa.adoc[] page.
...
For more information, see the xref:3.x@typeql::statements/has.adoc[] page.

=== Modifiers

`offset`::
Offset the result of a query by the specified number of results. +
See an xref:3.x@typeql::modifiers/pagination.adoc[example].
...
See an xref:3.x@typeql::modifiers/sorting.adoc[example].

=== Logic

`or`::
Specify a disjunction between two blocks of statements surrounded by curly brackets. +
For more information, see the xref:3.x@typeql::patterns/disjunction.adoc[] page.

`not`::
Specify a negation of a block of statements surrounded by curly brackets. +
For more information, see the xref:3.x@typeql::patterns/negation.adoc[] page.

=== Value comparators

`==`, `!=`, `>`, `<`, `>=`, `+<=+`, `like`, `contains`::
Specify the result of comparison to be true. +
For more information, see the xref:3.x@typeql::values/comparators.adoc[] page.

=== Aggregation

`group`::
Group results by a given variable. +
For more information, see the xref:3.x@typeql::modifiers/grouping.adoc[] page.

`count`::
Count the number of results. +
For more information, see the xref:3.x@typeql::modifiers/aggregation.adoc#_count[Aggregation] page.

`max`::
Find the maximum value of a given variable. +
For more information, see the xref:3.x@typeql::modifiers/aggregation.adoc[Aggregation] page.
