...
Value variables are used in patterns with xref:3.x@typeql::values/arithmetic.adoc[arithmetics] or
xref:3.x@typeql::values/functions.adoc[built-in functions] to represent values in statements.
...
Value variables can store a value of any xref:3.x@typeql::values/value-types.adoc[value type] supported by attributes.
