...
* a xref:3.x@typeql::concepts/concept-variables.adoc[concept variable] representing an attribute (e.g., `$size-kb`),
* a xref:3.x@typeql::values/value-variables.adoc[value variable] (e.g., `?sum`),
* a literal value (e.g., `4`),
* an xref:3.x@typeql::values/arithmetic.adoc[arithmetic operation] (e.g., `+`),
* or a xref:3.x@typeql::values/functions.adoc[built-in function] (e.g., `max()`).
...
Arithmetic operations are often used in a combination with xref:3.x@typeql::values/comparators.adoc[comparators] and
