...
xref:partial$fundamentals.adoc#_attribute_types[same instance] of the `certified-fortune-teller` attribute type
...
To perform aggregation in TypeDB, we first write a xref:queries/queries.adoc#_match[match clause] to describe the set of
