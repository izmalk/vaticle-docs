...
:page-aliases: 2.x@typeql::data/insert.adoc
...
----
// end::syntax[]
where `<insert_pattern>` is a pattern of valid data-specific TypeQL statements including xref:2.x@typeql::statements/isa.adoc[],
xref:2.x@typeql::statements/has.adoc[],
xref:2.x@typeql::statements/role-assignment.adoc[role assignment], and xref:2.x@typeql::statements/value-assignment.adoc[value assignment].

The _optional_ `match` clause `<pattern>` can be any valid match pattern: for more information, see the
xref:2.x@typeql::patterns/overview.adoc[Patterns] and xref:2.x@typeql::statements/overview.adoc[Statements] sections.
...
xref:2.x@typeql::statements/isa.adoc[isa] statements.
* Ownerships of attributes are inserted using a xref:2.x@typeql::statements/has.adoc[has] statement. Note that inserting an ownership of an attribute that doesn't yet exist in a database (with a new value), implicitly inserts such an attribute.
* Role players are usually assigned as a part of a <<_inserting_relations,relation insertion>> with a
xref:2.x@typeql::statements/role-assignment.adoc[role assignment] statement.
// end::insert_clause[]
...
include::2.x@2.x@typeql::partial$iam-database-links.adoc[]
...
To insert an entity, use a concept variable followed by an xref:2.x@typeql::statements/isa.adoc[isa]
...
or combine statements into a xref:2.x@typeql::statements/overview.adoc#_composite_statements[composite statement].
...
You can also use the xref:2.x@typeql::statements/key.adoc[@key] or
xref:2.x@typeql::statements/unique.adoc[@unique] annotations for an attribute type ownership schema definition
...
You can specify the value with a xref:2.x@typeql::values/comparators.adoc[comparator] or
xref:2.x@typeql::statements/value-assignment.adoc[] statement.
...
xref:2.x@typeql::queries/delete.adoc#_orphaned_attributes[orphaned attributes]
and aren't very useful to store in a database.

To implicitly insert an attribute, assign an ownership of it with a xref:2.x@typeql::statements/has.adoc[has] statement.
...
To insert an ownership of an attribute, use a xref:2.x@typeql::statements/has.adoc[] statement:
...
Use a xref:2.x@typeql::statements/role-assignment.adoc[] statement with an
xref:2.x@typeql::statements/isa.adoc[isa] statement in an `insert clause` to insert a relation.
...
match the relation from a database and use a xref:2.x@typeql::statements/role-assignment.adoc[] statement
with only new roleplayers.
Don't use an xref:2.x@typeql::statements/isa.adoc[] statement with relation type
