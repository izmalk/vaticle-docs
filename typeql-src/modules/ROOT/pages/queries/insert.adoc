...
(use xref:schema/types.adoc[define] to insert new types) and can't have the following:
...
(which adds a xref:schema/types.adoc#_key_attribute[uniqueness constraint]), the query would succeed the first
...
of matched results. In our xref:typedb:ROOT:quickstart-guide.adoc#_insert_data[IAM dataset], there is only one match.
...
For more information, see the xref:queries/match.adoc[] page.
====
