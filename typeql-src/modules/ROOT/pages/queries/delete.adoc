...
:page-aliases: 3.x@typeql::data/delete.adoc
...
----
where `<delete_pattern>` is a pattern of valid data-specific TypeQL statements including xref:3.x@typeql::statements/isa.adoc[],
...
xref:3.x@typeql::statements/isa.adoc[isa] statement in an `insert` clause.
* To delete ownership of an attribute, use a xref:3.x@typeql::statements/has.adoc[has] statement.
* Role players can be deleted with a
xref:3.x@typeql::statements/role-assignment.adoc[role assignment] statement. See <<_deleting_role_players_from_a_relation>>.
...
xref:3.x@typeql::statements/has.adoc[] pattern in the `delete` clause of the same query:
...
[NOTE]
====
We are not using an xref:3.x@typeql::statements/isa.adoc[] statement in the `delete` clause above
...
that needs to stop playing a role in it, then use xref:3.x@typeql::statements/role-assignment.adoc[] statement in the
...
[NOTE]
====
We are not using an xref:3.x@typeql::statements/isa.adoc[] statement in the `delete` clause
