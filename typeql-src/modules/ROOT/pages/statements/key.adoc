...
xref:3.x@typeql::statements/owns.adoc[owns statement] to specify, that the selected attribute type serves as a key for the subject type.

== Syntax

The syntax of a `@key` statement includes:

* xref:3.x@typeql::statements/owns.adoc[owns statement]
* the `@key` keyword
...
The `@key` keyword adds a xref:3.x@typeql::statements/unique.adoc[unique annotation] constraint,
