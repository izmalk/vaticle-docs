...
where each `<pattern>` is _any valid pattern_ and we added `;` to emphasize that patterns _always end_ with `;`. Note, each `<pattern>` itself may be a conjunction of smaller patterns or individual statements: see xref:2.x@typeql::patterns/matching.adoc[patterns] for more information.
...
[cols-2]
--
.xref:2.x@typeql::patterns/disjunction.adoc[]
[.clickable]
****
Learn more about disjunction in TypeQL.
****

.xref:2.x@typeql::patterns/negation.adoc[]
[.clickable]
****
Learn more about negation in TypeQL.
****

.xref:2.x@typeql::statements/overview.adoc[]
[.clickable]
****
