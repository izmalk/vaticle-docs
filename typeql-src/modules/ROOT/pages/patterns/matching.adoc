...
:page-aliases: 3.x@typeql::data/basic-patterns.adoc, typedb::basics/patterns.adoc, 3.x@typeql::data/match.adoc
...
A pattern consists of one or multiple xref:3.x@typeql::statements/overview.adoc[statements], combined by any combination of
xref:3.x@typeql::patterns/conjunction.adoc[conjunctions],
xref:3.x@typeql::patterns/disjunction.adoc[disjunctions],
and xref:3.x@typeql::patterns/negation.adoc[negations].

A matching pattern can use any valid TypeQL statements,
except the xref:3.x@typeql::statements/rule.adoc[rule] definition statement.
...
* an exact value for every xref:3.x@typeql::values/value-variables.adoc[value variable]
* an exact concept for every xref:3.x@typeql::concepts/concept-variables.adoc[concept variable].
...
xref:3.x@typeql::statements/overview.adoc#_composite_statements[composite statement] that produces the same effect.

It may be easier to find a specific entity using its attributes, if there is the
xref:3.x@typeql::statements/unique.adoc[unique] or xref:3.x@typeql::statements/key.adoc[key] annotation set in the schema.
...
.xref:3.x@typeql::queries/overview.adoc[]
[.clickable]
****
