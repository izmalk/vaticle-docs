...
xref:3.x@typeql::concepts/types.adoc[type] from the schema of the database.
...
that originate from different xref:3.x@typeql::concepts/types.adoc[root types]
...
The value of an attribute must be of the correct xref:3.x@typeql::values/value-types.adoc[Value type]
...
Given the schema from the xref:3.x@typeql::concepts/types.adoc#_example[Types page example],
