...
xref:schema/types.adoc#_define_entity_types[Define entity types] section on the
xref:schema/types.adoc[] page.
...
We have to xref:schema/types.adoc#_overrides_a_role[override]
...
xref:schema/types.adoc#_define_relation_types[Define relation types] section on the
xref:schema/types.adoc[] page.
...
xref:schema/types.adoc#_value_types[list of value types] on the
xref:schema/types.adoc[] page.
...
xref:schema/types.adoc#_define_attribute_types[Define attribute types] section on the
xref:schema/types.adoc[] page.
...
xref:schema/types.adoc[defined] in the schema of the database.
...
The simplest xref:queries/insert.adoc[Insert query] can look like this:
...
Now to read that data, we can use the xref:queries/get.adoc[Get] query:
...
xref:queries/get.adoc#_number_of_answers[Get query] page or the
xref:queries/get.adoc#_explanation_of_filter[example explanation].
...
Learn more essential information about different queries in TypeQL with the xref:queries/queries.adoc[] page.

Or skip it and go straight to the Schema and Data sections of this documentation for more in-depth look on every query
type.

Use the xref:keywords.adoc[] page for factual information about syntax of TypeQL.
