= TypeDB Core installation guide
:tabs-sync-option:
:experimental:
// Goal – Install TypeDB Core

TypeDB Core can be installed in multiple ways:
using <<_docker,Docker>> image,
via <<_package,package manager>>, or by
<<_manual,manual installation>>.
For self-hosted TypeDB Cloud installation and setup,
see the xref:home::install/cloud-self-hosted.adoc[Self-hosted deployment] page.

[#_docker]
== Docker

include::2.x@home::partial$docker.adoc[tag=install]

=== Run

include::2.x@home::partial$docker.adoc[tags=run]

include::2.x@home::partial$docker.adoc[tags=run-info]

include::2.x@home::partial$docker.adoc[tag=stop]

include::2.x@home::partial$docker.adoc[tag=start]

[#_package]
== Package manager

[tabs]
====
macOS::
+
--
include::2.x@home::partial$macos.adoc[tag=install-homebrew]
--

Linux::
+
--
include::2.x@home::partial$linux.adoc[tag=install-apt]
--

Windows::
+
--
No package manager option is available for Windows. See the <<_manual>> section below.
--
====

=== Run

// tag::core-run[]
To start a local TypeDB Core server:

[source,console]
----
$ typedb server
----
// end::core-run[]
// tag::core-stop[]
To stop a local TypeDB server, close the terminal where it runs or press kbd:[Ctrl+C].
// end::core-stop[]

[#_manual]
== Manual install

Download the latest version of TypeDB Core from the table below.

[cols="^.^1,^.^1,^.^1,^.^1",caption="",options="header"]
|===
|Release notes |macOS |Linux |Windows

include::2.x@manual:resources:partial$typedb-all-latest-links.adoc[]
|===

For all versions,
see the link:https://cloudsmith.io/~typedb/repos/public-release/packages/?q=typedb-all&sort=-version[Packages] page.

[tabs]
====
macOS::
+
--
include::2.x@home::partial$macos.adoc[tag=manual-install]
--

Linux::
+
--
include::2.x@home::partial$linux.adoc[tag=manual-install]
--

Windows::
+
--
include::2.x@home::partial$win.adoc[tag=manual-install]
--
====

=== Run

include::2.x@home::install/core.adoc[tags="core-run,core-stop"]

== Learn more

[cols-2]
--
.xref:home::install/studio.adoc[]
[.clickable]
****
Install an IDE that facilitates the development process for TypeDB.
****

.xref:home::quickstart.adoc[]
[.clickable]
****
Connect to TypeDB and create a simple sample database.
****

.xref:home::crash-course.adoc[]
[.clickable]
****
Learn about the basics of TypeDB.
****

.xref:learn::course-overview.adoc[Learning course]
[.clickable]
****
Check out our dedicated Learning course for TypeDB.
****
--
