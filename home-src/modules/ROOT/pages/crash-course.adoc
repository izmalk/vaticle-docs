...
:page-aliases: 2.x@home::25-queries.adoc
...
A xref:2.x@typeql::queries/fetch.adoc[Fetch query]
...
[,typeql]
----
include::2.x@home::partial$cc-queries/subquery.tql[lines=1..4]
----

The above example uses xref:2.x@typeql::queries/fetch.adoc#_customize_output[customized output labels] for the JSON output.

A Fetch query can be used with a xref:2.x@typeql::queries/fetch.adoc#_subqueries[subquery].
For example, let's add a subquery to fetch paths for all files that the user has access to:

[,typeql]
----
include::2.x@home::partial$cc-queries/subquery.tql[]
----

[#_get_queries]
=== Get queries

A xref:2.x@typeql::queries/get.adoc[Get query] returns stateful objects representing concepts matched by selected variables
...
[,typeql]
----
include::2.x@home::partial$cc-queries/get-count-files.tql[lines=1..3]
----
...
An xref:2.x@typeql::queries/insert.adoc[Insert query] inserts data into a database.
...
[,typeql]
----
include::2.x@home::partial$cc-queries/insert.tql[]
----
...
[,typeql]
----
include::2.x@home::partial$cc-queries/match-insert.tql[]
----
...
A xref:2.x@typeql::queries/delete.adoc[Delete query] deletes data from a database.
...
[,typeql]
----
include::2.x@home::partial$cc-queries/delete.tql[]
----
...
[,typeql]
----
include::2.x@home::partial$cc-queries/update.tql[]
----
...
TypeQL uses declarative xref:2.x@typeql::patterns/overview.adoc[patterns] to seek and manipulate data in a database.
...
[,typeql]
----
include::2.x@home::partial$cc-queries/list-users.tql[]
----

The above is a xref:2.x@typeql::queries/fetch.adoc[Fetch query],
...
TypeQL patterns consist of TypeQL xref:2.x@typeql::statements/overview.adoc[statements],
which can be combined in any order by an implicit xref:2.x@typeql::patterns/conjunction.adoc[conjunction] (i.e., a logical "and") and other pattern operations like `or` (a logical "or") or `not` (a logical negation).
Let's modify the previous query's pattern to retrieve a specific user who has name `Bob`:

.Composable pattern example
[,typeql]
----
include::2.x@home::partial$cc-queries/fetch-all-bob-emails.tql[]
----
...
xref:2.x@typeql::values/comparators.adoc#_equality_comparator[equal] to `Bob`. This is an example of a composition of patterns with a logical "and", but simply reading the query out loud should make it clear what is going on!
...
xref:2.x@typeql::statements/plays.adoc[plays] statement.
...
[,typeql]
----
include::2.x@home::partial$cc-queries/n-relations.tql[]
----
...
Creating an attribute is just instantiating an attribute type with a value of the appropriate value type. You can create an attribute explicitly by using an xref:2.x@typeql::statements/isa.adoc[isa statement] with
xref:2.x@typeql::statements/value-assignment.adoc[value assignment]:
...
[,typeql]
----
include::2.x@home::partial$cc-queries/attributes.tql[lines=6..7]
----

You can limit behavior this by using the xref:2.x@typeql::statements/key.adoc[key statement],
...
[,typeql]
----
include::2.x@home::partial$cc-queries/attributes.tql[lines=9..12]
----
...
[,typeql]
----
include::2.x@home::partial$cc-queries/inheritance-polymorphism.tql[]
----
...
For more information on the `isa` and `isa!` keywords, see the xref:2.x@typeql::statements/isa.adoc[] page.
...
[,typeql]
----
include::2.x@home::partial$cc-queries/interface-polymorphism.tql[]
----
...
[,typeql]
----
include::2.x@home::partial$cc-queries/inference-rule.tql[]
----
...
[,typeql]
----
include::2.x@home::partial$cc-queries/inference-fetch.tql[]
----
