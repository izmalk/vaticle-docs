...
TypeDB uses the xref:learn::9-modeling-schemas/9.1-the-pera-model.adoc[PERA model], a novel conceptual data model based on recent https://typedb.com/papers/typeql-theory[research]. The model aims to provide higher levels of flexibility and expressivity when working with structured data. The model underpins the type system of TypeDB and its pattern-based xref:3.x@typeql::overview.adoc[query language TypeQL].
...
A xref:3.x@typeql::queries/fetch.adoc[Fetch query]
...
The above example uses xref:3.x@typeql::queries/fetch.adoc#_customize_output[customized output labels] for the JSON output.

A Fetch query can be used with a xref:3.x@typeql::queries/fetch.adoc#_subqueries[subquery].
...
A xref:3.x@typeql::queries/get.adoc[Get query] returns stateful objects representing concepts matched by selected variables
...
An xref:3.x@typeql::queries/insert.adoc[Insert query] inserts data into a database.
...
A xref:3.x@typeql::queries/delete.adoc[Delete query] deletes data from a database.
...
TypeQL uses declarative xref:3.x@typeql::patterns/overview.adoc[patterns] to seek and manipulate data in a database.
...
The above is a xref:3.x@typeql::queries/fetch.adoc[Fetch query],
...
TypeQL patterns consist of TypeQL xref:3.x@typeql::statements/overview.adoc[statements],
which can be combined in any order by an implicit xref:3.x@typeql::patterns/conjunction.adoc[conjunction] (i.e., a logical "and") and other pattern operations like `or` (a logical "or") or `not` (a logical negation).
...
xref:3.x@typeql::values/comparators.adoc#_equality_comparator[equal] to `Bob`. This is an example of a composition of patterns with a logical "and", but simply reading the query out loud should make it clear what is going on!
...
xref:3.x@typeql::statements/plays.adoc[plays] statement.
...
Creating an attribute is just instantiating an attribute type with a value of the appropriate value type. You can create an attribute explicitly by using an xref:3.x@typeql::statements/isa.adoc[isa statement] with
xref:3.x@typeql::statements/value-assignment.adoc[value assignment]:
...
You can limit behavior this by using the xref:3.x@typeql::statements/key.adoc[key statement],
...
For more information on the `isa` and `isa!` keywords, see the xref:3.x@typeql::statements/isa.adoc[] page.
