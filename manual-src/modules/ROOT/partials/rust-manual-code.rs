    ...
    }
    {
        ...
        let db = databases.get(DB_NAME)?;
        {
            let session = Session::new(db, SessionType::Schema)?;
            {
                ...
                let condition = 3.x@typeql::parse_pattern("{$u isa user, has email $e; $e contains '@typedb.com';}")?
                    .into_conjunction();
                let conclusion = 3.x@typeql::parse_pattern("$u has name 'Employee'")?.into_statement();
