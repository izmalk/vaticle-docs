...
A Delete query is used to delete _data_ from a TypeDB database. A `delete` clause is executed once per every result matched by the `match` clause of the same Delete query. For a detailed explanation of the query see the xref:3.x@typeql::queries/delete.adoc[] page.
...
[cols-2]
--
.More about a xref:3.x@typeql::queries/delete.adoc[]
[.clickable]
****
