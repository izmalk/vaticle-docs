...
It has a built-in xref:3.x@typeql::overview.adoc[TypeQL] support, as well as Type Browser, Type Editor,
...
In an empty database, there are only three xref:3.x@typeql::concepts/types.adoc#_type_hierarchy[root types] shown in the panel.
...
For example,
we can't xref:3.x@typeql::queries/undefine.adoc#_undefine_a_type_with_a_subtype[delete a type that has a subtype].
// end::type-editor[]
