...
A Undefine query is used to delete type and rule definitions from a schema. For a detailed explanation of the query see the xref:3.x@typeql::queries/undefine.adoc[] page. For an alternative way of editing your schema, see the xref:manual::defining/schema-editing.adoc[] page.
