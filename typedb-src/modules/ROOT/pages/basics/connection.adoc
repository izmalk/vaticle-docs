...
This page describes how to connect to a TypeDB to send a query.
For an examples with different TypeDB Clients, see the <<_examples,Examples>> section below.
...
Every TypeDB Client instance supports a single network connection to a TypeDB server or cluster,
but can support multiple sessions opened at the same time.
Every session can support multiple transactions, and every transaction -- multiple queries.
