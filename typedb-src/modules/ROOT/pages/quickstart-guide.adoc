...
Use the following command to Run TypeDB Console and connect to a server, located at the default address of
`localhost:1729`:
...
Upon establishing a connection to a database, TypeDB Console shows a command prompt with `>` symbol.

=== Create a new database

To create a new database with name `try-iam` use the following command with TypeDB Console prompt:
