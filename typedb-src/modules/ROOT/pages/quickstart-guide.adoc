...
Quickstart guide includes instructions for the following steps:
...
This guide covers both TypeDB Console and TypeDB Studio, which are two of the TypeDB Clients available for TypeDB.
...
To proceed, please make sure you have TypeDB server (or cluster) xref:install-and-run.adoc#_install[installed].
...
=== Download schema and data files for a database
