...
By default, the IAM dataset used in the xref:root/quickstart-guide.adoc[Quickstart guide] sets permissions only for the
`modify_file` action.
...
xref:root/quickstart-guide.adoc[Quickstart guide]) can infer `view_file` permission for multiple files for a single
...
the result of 10 files (by default in the IAM database from the xref:root/quickstart-guide.adoc[Quickstart guide]).
