...
The following tips might be useful for new users:

* A database's schema defines a vocabulary for queries.
+
Type labels are used in queries. Try naming types with short but precise names.
Simple and meaningful type labels make queries ease to read and develop.
+
...
* If you have a problem with starting the design of a new database schema, try the following order:
** Define main entity types and hierarchy between them.
** Define relations with roles and what types can play what roles.
** Define attribute types, set their values types and assign ownership over these attributes.
** Iterate by updating, testing, and refactoring your schema.
...
But this represents an incomplete information state.
+
For example, a marriage in which only one of the spouses is known.
+
...
over *inheritance* (using subtyping to describe those functions).
+
This is because a type `a` should be a subtype of `b` only if every instance of `a` is necessarily also an instance
of `b` (within the domain).
+
...
The following tips might be helpful for new users:
...
the problem lies within the query itself or other parts of your application's logic, e.g., connection control.
TypeDB Studio is thoroughly tested, can visualize query results, and manages a DB connection (sessions and transactions).
...
* A query pattern describes constraints on data to retrieve/process, but not the way to retrieve it.
+
TypeDB query engine will try to retrieve/process all required concepts in the most optimal and efficient way.
But the same result can be achieved with different sets of constraints.
Each schema construct incurs a computational cost when traversed as part of a query.
+
Sorted in the order of
*increasing* cost, the schema constructs are:
+
    . Attributes
    . Entities
    . Subtypes
...
* TypeDB Studio and TypeDB Console output differs from TypeDB Driver output.
+
Both Studio and Console provide not a direct literal TypeDB response, but processed, refined, sometimes even enriched
data as output.
TypeDB Studio's `Log` tab output should be the same as TypeDB Console output.
+
For example, if we query for an instance of a relation type it will add information on all its role-players.
+
TypeDB Drivers require more accurate and manual approach to handle TypeDB responses. With the same query it will provide
only the data that was requested.
