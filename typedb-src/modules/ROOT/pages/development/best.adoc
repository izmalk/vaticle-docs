...
A `match` clause of a query does not only set context for execution of the query, but also defines the number of
...
[NOTE]
====
The only exception is an Insert query, as it can be without a `match` clause. In that case, the `insert` clause of such
an Insert query will be executed exactly once. In any case, insert query returns inserted concepts as a response.
====
...
If we try to insert ownership of an attribute type instance that was not inserted before, we will achieve that by
implicitly creating the attribute instance we are assigning ownership of.

This contrasts with an SQL model where "attributes" are columns in a table, and values can be repeated in each row.

When combined with how pattern matching works -- it can create interesting, but surprising results.
...
`email`. It filters theresults only to contain `$e` – instances of the `email` type.
...
Having an ownership of an attribute with an empty string value is not the same as not having an ownership of any
...
any type, it’s also important not to blindly delete the attributes it owns, as those attributes might also be owned
by other entities. The easiest approach is to allow attributes to become orphaned (not owned by anyone) and then
...
The occurrence of `attribute` in this query can also be swapped for another attribute type name so as only to clean up
orphaned attributes of that type.
...
When writing a rule that implements transitivity, if the rule is written naively, then sometimes it can lead to
...
The forwards transitive form should be used where queries normally search for players of `role1` based on a given
player of `role2`.

The backwards transitive form should be used where queries normally search for players of `role2` based on a given
