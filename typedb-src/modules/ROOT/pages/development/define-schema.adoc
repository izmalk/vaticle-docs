...
Types in a schema define a data model, its constraints, and a vocabulary used in TypeQL queries to the database.

Rules define embedded logic for reasoning engine to be able to infer new relations or attribute ownerships.

== How to define a schema

To define or undefine anything in a TypeDB database schema, we need:

1. Prepare a TypeQL string for Define query.
2. Connect to TypeDB with
