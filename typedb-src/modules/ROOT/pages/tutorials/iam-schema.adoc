...
[NOTE]
====
The terminology used in the IAM xref:root/quickstart-guide.adoc#_iam_schema[schema] is defined
...
Check our xref:root/quickstart-guide.adoc[Quickstart guide] for step-by-step instructions how to load the schema and data to a
TypeDB database.
====
...
The sample dataset that we loaded in the xref:root/quickstart-guide.adoc#_iam_microdataset[Quickstart guide]
