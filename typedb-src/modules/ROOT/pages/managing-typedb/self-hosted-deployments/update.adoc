...
xref:managing-typedb/self-hosted-deployments/configuration.adoc#_the_default_location_of_the_config_file[config]) into the data directory of the new
TypeDB server installation.

Alternatively, just update the new server's
xref:managing-typedb/self-hosted-deployments/configuration.adoc#_the_default_location_of_the_config_file[configuration file] to use the same
*data directory* path as the old one. That is why we xref:managing-typedb/self-hosted-deployments/configuration.adoc#_storage_configuration[recommend]
...
Please see the xref:managing-typedb/self-hosted-deployments/export-import.adoc[Export/import] page for information on how to do that.
