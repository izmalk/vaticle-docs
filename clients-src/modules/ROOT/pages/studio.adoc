...
In the `Connect to TypeDB` window click the btn:[Disconnect] button.

[#_create_a_database]
=== Create a database

To create a new database make sure TypeDB Studio is connected to a TypeDB server and click the
...
click the btn:[Open Project] button in the *Projects* panel on the left (under the database icon).
...
* Right-click on the type in the *Type browser* list. Click the *Rename Type* option. Edit the name in the Label
...
* Open the type editor window by double-clicking on the type. At the very bottom click the btn:[Delete] button and
